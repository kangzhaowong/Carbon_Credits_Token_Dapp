...
contract LeafContract is ERC20, AccessControl {
    ...
    bytes32 public constant PARTNER_COMPANY_ROLE = keccak256("PARTNER_COMPANY_ROLE");
    ...
    mapping (string => uint256) public permit_value;
    mapping (string => bytes32[]) public permit_answers;
    ...
    function usePermit(string memory _permit_id, string memory _code) public returns (bool) {
        require(!hasRole(PARTNER_COMPANY_ROLE, msg.sender),"INVALID PERMISSIONS");
        require(permit_value[_permit_id]!=0,"INVALID CODE");
        require(permit_answers[_permit_id].length!=0,"INVALID CODE");
        for (uint256 i=0;i<permit_answers[_permit_id].length;i++) {
            require(keccak256(abi.encodePacked(_code,Strings.toString(i))) == permit_answers[_permit_id][i],"INVALID CODE");
        }
        _mint(msg.sender,permit_value[_permit_id]);
        delete permit_answers[_permit_id];
        delete permit_value[_permit_id];
        return true;
    }

    function exchangeTokens(address _company_address, uint256 _tokens) public payable returns (uint256) {
        require(balanceOf(msg.sender) >= _tokens, "INSUFFICIENT TOKENS");
        require(!hasRole(PARTNER_COMPANY_ROLE, msg.sender),"INVALID PERMISSIONS");
        require(_tokens*company_data[_company_address].exchange_rate +  (_tokens*company_data[_company_address].exchange_rate)/inverse_royalty_percent <= company_funds[_company_address],"INSUFFICIENT FUNDS");
        transfer(_company_address,_tokens);
        uint256 amount = _tokens*company_data[_company_address].exchange_rate;
        uint256 royalty_amount = (_tokens*company_data[_company_address].exchange_rate)/inverse_royalty_percent;
    ...
    function newPermit(string memory _permit_id, bytes32[] memory _answers, uint256 _amount) public returns (bool) {
        require(hasRole(PARTNER_COMPANY_ROLE, msg.sender),"INVALID PERMISSIONS");
        permit_value[_permit_id] = _amount;
        permit_answers[_permit_id] = _answers;
        return true;
    }
    ...
    function withdrawOwnerBalance(uint256 _amount) public returns (bool) {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender),"INVALID PERMISSIONS");
    ...
    function addPartnerCompany(address _company_address, string memory _company_name, uint256 _rate) public returns (bool) {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender),"INVALID PERMISSIONS");
        _grantRole(PARTNER_COMPANY_ROLE,_company_address);
        company_data[_company_address] = CompanyData({company_name: _company_name, exchange_rate: _rate});
        company_addresses.push(_company_address);
        return true;
    }

    function removePartnerCompany(address _company_address) public returns (bool) {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender),"INVALID PERMISSIONS");
        _revokeRole(PARTNER_COMPANY_ROLE,_company_address);
        for (uint256 i=0;i<company_addresses.length;++i) {
            if (_company_address == company_addresses[i]) {
                company_addresses[i] == company_addresses[company_addresses.length-1];
